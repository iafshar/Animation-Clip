...
char Tokenizer::GetChar() {  //gets the char and advances the reading to next character
    char c = char(getc((FILE *)File));
    if (c == '\n') LineNum++;
    return c;
}

char Tokenizer::CheckChar() {  //just looks at what the character is does not advance the reading
...
bool Tokenizer::GetToken(char *str) {  //pass in Key from current dictionary then 
    SkipWhitespace();
